...
export function computeOrbitPoints(
    ...
    const currentState = keplerToCartesian(elements, now);
    let currentM = elements.M + Math.sqrt(mu / Math.pow(elements.a, 3)) * (now - elements.epoch) / 86400000 * 180 / Math.PI; // M atual em deg
